        ...
        <main class="max-w-5xl px-5 mx-auto pt-7 lg:px-0">
            ...
            <div class="flex flex-col items-center justify-between mt-5 space-y-3 md:space-y-0 md:flex-row">
                <div class="relative w-full md:mr-10 grow">
                    ...
                    <input placeholder="Search books..." v-model="search" type="search" name="search" id="search"
                        class="pl-11 input" />
                </div>
                <button @click="bookModal.openModal()" class="w-full py-3.5 md:py-2.5 md:w-auto btn shrink-0">
                    Add Book
                </button>
            </div>
            ...
            <div class="relative mt-5 border border-gray-100 rounded-lg">
                <ClientOnly>
                    <EasyDataTable :search-value="search" empty-message="No Book Found" theme-color="#f97316"
                        table-class-name="eztble" :headers="headers" :items="bookStore.books">
...
await useAsyncData("allBooks", () => bookStore.getAll(), {
